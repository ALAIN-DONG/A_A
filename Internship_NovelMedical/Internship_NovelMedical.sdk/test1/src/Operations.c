...
//修改指定adc的测试值（模式自动切换为测试模式）
//change tested value in specified ADC (the function mode will be changed automatically into test mode)
...
void init_spi(void){
	...
	Write_reg(0x0109, 0x03);//set sync clock
...
//初始化所有色弱的是模块，并自动校准，进行位对齐
//initial all serdes modules, auto calibrate to realize bit alignment
...
//最终检查每路信号是否正确，并生成报告
//final check for every channel and generate a report
...
void Execute_Order_New(int order_num, u32 New_Order_Addr, u32 New_Order_Data, u32 New_Operation, int channel, char *feedback){
    ...
    switch (order_num){
    		...
    		case		60:	switch(New_Operation){//D
    								...
    								case	0x051:	SetValue_add_times(New_Order_Data&0x7FF);
    		...
    		case		70:	switch(New_Operation){//A
    								...
    								case	0x040:	auto_init_serdes();
    												final_check();
    												sprintf(feedback, "auto configure serdes for all channels and result report generated\r\n");
    												break;
