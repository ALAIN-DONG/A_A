...
#define MAX_COUNT_Auto_run_Flag (1*MULT_SPEED/100)
int count_Auto_run_Flag = 0;
volatile int Auto_run_Flag = 0;

#define MAX_COUNT_auto_transmission_Flag (1*MULT_SPEED/10000)
int count_auto_transmission_Flag = 0;
volatile int auto_transmission_Flag = 0;

#define MAX_COUNT_Sampling_Flag (1*MULT_SPEED/625)  //除数：加快的倍数： 越高-->越快
...
static void TimerIntrHandler(void *CallBackRef)
{
	...
	if (++count_auto_transmission_Flag >= MAX_COUNT_auto_transmission_Flag){
		count_auto_transmission_Flag = 0;
		auto_transmission_Flag = 1;
	}
	...
	if (++count_Auto_run_Flag >= MAX_COUNT_Auto_run_Flag){
		count_Auto_run_Flag = 0;
		Auto_run_Flag = 1;
	}
